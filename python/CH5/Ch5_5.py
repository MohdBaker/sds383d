...
def sample_z(y,m1,m0,sigma1,sigma0,alpha, beta, n):
	z=np.zeros((n,1))
	for i in range (n):
		alpha_n=alpha+np.sum(z)-z[i]
		beta_n=beta+n-(np.sum(z)-z[i])-1
		p1=alpha_n*np.exp(-1*(y[i]-m1)**2/(2*sigma1))
		p0=beta_n*np.exp(-1*(y[i]-m0)**2/(2*sigma0))
		if p1>p0:
			z[i]=1
		else:
			z[i]=0
...
z0=np.zeros(y.shape)
z0[ind1]=1
def sampler(y,mu0,a0,k0,b0,z0,alpha,beta,n,n_samples=1000):
	...
	for i in range(1,n_samples):
		...
		z[:,i]=sample_z(y,m1[i],m0[i],sigm1[i],sigm0[i],alpha,beta, n)
	return m0,m1,z


m0,m1,z=sampler(y,mu0,a0,k0,b0,z0,alpha,beta,n,1500)
